...
const Slider: React.FC<SliderProps> = ({
  ...
  const pointerX = useRef(new Animated.Value(0));
  ...
  const updateThumbs = useCallback(() => {
    ...
    updateSelectedRail && updateSelectedRail();
  ...
  }, [highProp, inPropsRefPrev, updateThumbs, lowProp]);
  ...
  const onPanResponderGrant = useCallback(
    (
      ...
      pointerX.current.removeAllListeners();
      pointerX.current.addListener(({value: pointerPosition}) => {
  ...
  const onPanResponderMove: (
    ...
    (...args) => {
      if (!disabled) {
        Animated.event([null, {moveX: pointerX.current}], {
  ...
  const panResponderFactory = useRef(
    new PanResponderFactory({
      onPanResponderGrant,
  ...
  useEffect(() => {
    panResponderFactory.current.updateValues({
      onPanResponderGrant,
      onPanResponderRelease,
      onPanResponderMove,
    });
  }, [onPanResponderGrant, onPanResponderRelease, onPanResponderMove]);

  const {panHandlers} = panResponderFactory.current.usePanResponder();
